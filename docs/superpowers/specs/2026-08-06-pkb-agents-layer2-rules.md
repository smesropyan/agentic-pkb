...
| SK-16 | Layer 2's source ordering must **agree with `pkb.core.resolve_skills`**: for every topic, the winning skill set the harness loads equals `resolve_skills(kb_root, topic_path)` unioned with the packaged defaults not shadowed by a KB copy. Two independent implementations of one precedence rule are pinned by a property test. | layer1 PA-14; recon §6 | error | Property test over generated KB trees. **no key** |
...
| **C8** | **Does `voice` ship?** README Part 3 step 1's shipped list omits it; step 2 says `voice.md` is drafted from the human's writing; §2.4 says every draft is written in this voice — so something must exist from turn one. | **Ship `voice` as a bootstrapping *procedure*** — gather samples → draft the profile → have the human correct it → refine as notes accumulate — with an explicit "no profile yet: mirror the human's phrasing, write plainly" fallback (SK-1, §6.6). | Satisfies both paragraphs without contradiction and makes the bootstrapping conversation a skill the human can rewrite, rather than hidden prompt text. A generic *voice profile* is rejected outright: it is precisely the thing README says only the human can author, and a stale-but-plausible default would shape every draft invisibly. |
| **C9** | **Delegated threads.** arch §4 L177-181's mechanism sentence vs verified `checkpoint_ns` behaviour. | **Amend arch §4** to say delegated work runs in a nested `checkpoint_ns` under the Librarian's thread; keep the consequences (D-6, LB-9). | The consequences arch draws are correct and should stand; the mechanism is not, and Layer 3 will build a table against it. |
...
stays single-line and factual), rendered derived files, or tool output. The shipped version is a
**bootstrapping procedure**, not a voice profile (C8): gather the human's existing writing samples →
draft the voice description → have the human correct it → refine as notes accumulate, through the same
propose-and-approve loop as everything else. A "no profile yet" fallback that instructs the agent to
mirror the human's own phrasing from the source material and otherwise write plainly. A hard boundary:
applying voice **never alters the factual content of a human-written note**; style edits to human
content are proposed, not applied. Once the human rewrites this skill, it becomes their actual voice
profile — the same file, adopted (SK-4).
