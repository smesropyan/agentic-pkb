...
**Status**: **Built**, except the Librarian's routing workflow (§1.4), which was specified on
2026-08-07 after a live measurement and is **designed but unproven**. Every open question has a
default applied; none blocks the build.
...
| **built** | Implemented and covered by the passing suite (1127 tests, ruff, `mypy --strict`, three import contracts). |
| **designed** | Specified here, not yet implemented, and therefore not yet contradicted by anything. Every `LB-*` rule and the routing rows of the `RT`/`RG`/`PR` groups carry this marking as of 2026-08-07. |

A rule can be verified about the harness and still only designed as behaviour: `LB-14`'s derived
thread ids rest on a verified fact (the checkpointer keys on `thread_id` alone, D-6) and an unbuilt
mechanism.
...
| RT-58 | `run_scan(request) -> ScanResult` runs the topic's expert on **its own reserved thread** (`scan:<agent_id>:<uuid4>`), with the conflict-scan prompt, and returns the id so Layer 3 can exclude scan runs from the human's thread list. The scan's context never enters a human conversation. This is one of the two derived thread shapes (RT-36); the other, `<librarian-thread>::<agent-id>`, is the opposite case — Layer 3 should *show* it, as a child of the Librarian's thread (LB-9). | arch §7 L355-357, §4 L175-181 | warning | A scan run's thread id matches `^scan:`; the human's thread history is unchanged. **no key** |
...
**Marking.** Every row carries its own, because the split is not clean: LB-1 … LB-7 are `built`
apart from the clauses this rewrite added to LB-2 and LB-6, and LB-8 … LB-19 are `designed` —
the harness facts underneath them are verified and say so, the behaviour lands with `routing.py`.
...
| **D-24** | — (not in the arch doc) | **A middleware can remove a tool from a single graph.** deepagents' own `_ToolExclusionMiddleware` (`middleware/_tool_exclusion.py:45-54`) does exactly this: `wrap_model_call` filters `request.tools` and calls `request.override(tools=filtered)`. It is reachable only through the process-global harness profile, but the *mechanism* is per-request and available to any middleware. **Established by reading**, not by execution. | `KbRoutingMiddleware` uses the same `request.override(tools=…)` call to drop `task` on the Librarian (LB-12). Position in the stack does not matter for this: `request.tools` is assembled before the model call from every middleware's contribution, so any `wrap_model_call` sees the complete list. |
...
| **Q14** | **Does a routed expert thread show up in the human's thread list?** (LB-9, LB-14) | (a) yes, as a child of the Librarian thread it was derived from; (b) yes, flat, alongside the human's own expert conversations; (c) hidden until the reply offers it. | **(a)** (LB-9). The thread is real and holds real history, so hiding it makes "continue with the Cooking expert" a dead end when the human comes back an hour later and no longer has the reply in front of them. Flat listing is worse than either: after a week the expert's list is mostly fragments of Librarian turns with no visible parent. | Pure Layer 3 presentation over an id shape Layer 2 already derives. Cheap to change, but it is the difference between the offer being a link and being a sentence. |
...
*(The shipped `SKILL.md` still carries the pre-ruling text — see the open issues.)* When filing from the human's own input: restructure,
