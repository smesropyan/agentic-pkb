...
├── sources.py                   # LEAF: staging into <kb>/.inbox, extraction, sections (LS-7 … LS-9)
├── core/                        # Layer 1, built
└── agents/                      # Layer 2
    ...
    ├── ingestion.py             # the chunked ingestion loop + `ingest_source` (RT-60 … RT-64)
    ...
    └── skills/<name>/SKILL.md   # package data: the ten shipped skills (§6)
```
...
| RT-18 | Permissions are enforced **at the tool layer inside `FilesystemMiddleware`, never at the backend**. Layer 1's flush writing derived files directly on disk is therefore intended, not a violation: the deny list constrains the *agent* while the generators remain the sole writer. Corollary: no other `pkb.agents` code may write under `kb_root` **except from a named, argued-for site**. **Amended 2026-08-07 (large-source ingestion).** The corollary's intent was "no ad-hoc writers" — a middleware or a scan pass deciding to fix up an index — not "one writer forever", and LS-1 needs a second kind of writer that cannot go through a tool at all: the copy of the original is a *binary*, `write_file` takes text, and MW-7 intercepts exactly `write_file`/`edit_file`. So the allow-list is the three sites below plus Layer 1's delegated writers, and a fourth would need the same argument made again in the same place. **`skills.py`'s `adopt_skill`** (SK-4's carve-out, human-initiated and reachable by no agent tool). **`ingestion.py`'s `_write`** — the source file — and **`ingestion.py`'s `_copy_original`** — LS-1's copy — both harness code on a path no prompt can reach, both confined to `<topic>/references/<slug>/`, both under the write lock (RT-51). Two extra obligations make `_write` sanctionable rather than merely convenient: it calls `pkb.core.validate_content` and refuses on an error finding, so nothing lands that MW-13 would have refused, and it calls `gates.requires_approval` and refuses on a gate, so nothing lands that RT-21 would have stopped. Both record what they wrote into `kb_touched`, or MW-17 … MW-20's single flush never stamps or indexes the file (LS-1's second amendment). `pkb.sources.stage` also writes under `kb_root`, into `<kb>/.inbox/` (LS-8), and is outside this rule's reach only because the audit walks `pkb.agents`: it is a leaf module, the destination is invisible to every Layer 1 walk, and no generator maintains anything there. | recon §4 L233; arch I3; large-source ingestion, LS-1 | error | AST/grep: no `open(...,'w')`, `Path.write_text`, `shutil`/`os` tree mutation or `backend.write` in `pkb.agents` outside `{(skills.py, adopt_skill), (ingestion.py, _write), (ingestion.py, _copy_original)}`, and no reference to a Layer 1 write entry point outside `maintenance.py`'s `flush`, `tools/topics.py`'s scaffold and `runtime.py`'s `regenerate_all`. **no key** |
...
| ID | Rule | Source | Sev | Test assertion (API key?) |
|----|------|--------|-----|---------------------------|
| RT-21 | `requires_approval(tool_name, args, snapshot) -> GateReason \| None` is **one pure function** in `gates.py`, used both as the `when` predicate on the `interrupt_on` entry and by the description factory. "What gates" is table-testable without constructing a graph. The enum has **twelve** members, not the ten this table names. The eleventh is `UNRESOLVED_PATH = "unresolved-path"`, which fires when `canonical_kb_path` cannot resolve the target and **fails closed** for the six path-shaped gates that would otherwise evaluate against nothing. The twelfth is `REFERENCE_REWRITE = "reference-rewrite"`, added 2026-08-07 with large-source ingestion as RT-31's amended half. Both are human-visible — they travel to every client on `ActionView.reason` (§5.1) — so a client's approval vocabulary must carry them. | arch §9 L385-390 | error | Table test over ~20 (path, tool) rows asserting the expected `GateReason`; no graph, no model. An unresolvable path yields `UNRESOLVED_PATH` rather than `None`. **no key** |
...
| RT-25 | **GATE — new tags.** A write whose frontmatter introduces a `topic.*`/`domain.*` tag absent from `pkb.core.build_tag_tree(snapshot)` interrupts *before the file lands*. Layer 1 keeps no approved-tag list (TG-9/VA-40), so this gate is the **only** mechanical backing for README §1.5's "Do not create ad-hoc tags"; the `tag-proposal` skill shapes the dialog, the gate makes it unskippable. **Amended 2026-08-07 (large-source ingestion).** LS-3 has the ingestion loop collect the tags its sections are about, and the loop writes without a turn to be interrupted in — so it *does not propose through this gate*. It writes only tags `build_tag_tree` already knows and records an unknown one in the reading record as a proposal for the human. Same rule, one step earlier: a write carrying a new tag would gate and withhold the whole pass, which for a 300-page book means discarding hours of reading over a tag. | README §1.5 L189-191, §1.8 rule 7, §1.9 L442-444; large-source ingestion LS-3 | error | Re-using `topic.cooking.grilling` does not gate; adding `topic.cooking.sous-vide` gates once and the description names the new tag. An ingestion answering `TAGS:` with a known tag writes it and never gates; with an unknown one it writes neither the tag nor a gate, and the reading record names it. **no key** |
...
| RT-31 | **NO gate** on plain note ingestion (`notes/<t>.md`), on the **first** write of a reference depth file (`references/<src>/<src>.md`), or on any read. Capture must be frictionless (goal 2), the note carries the human's own words arriving through dialog, and a reference depth file is AI-generated on ingestion with nothing of the human's underneath it yet. **Amended 2026-08-07 (large-source ingestion).** The rationale above was written when a reference was written **once and never touched again**; re-ingestion makes a second pass over the same source a routine act, and an un-gated write then overwrites an extraction the human has already read and relied on. So the rule splits: the **first** write of a source file stays un-gated, and a write that **rewrites an existing** `references/<src>/<src>.md` gates as `GateReason.REFERENCE_REWRITE` with `["approve","edit","reject"]` — one proposal for the whole reconciled file, not one per argument. Without the split, "human content wins" holds for notes and quietly fails for everything derived from a source, which is most of what a knowledge base accumulates. **Where the line actually falls is LS-12's, and it is a diff rather than an authorship claim**: the write is un-gated for as long as it only *adds* — every existing body line surviving, in order — and gates the moment it would remove or alter one. That is what lets a re-ingestion's new argument, a new chapter section and another pass's reading record all land unattended (LS-12's first column) while a reworded argument stops (its second). Frontmatter is excluded, because the two frontmatter changes that matter already gate under RT-25 and RT-26 and the third (a conflict *flag*) must not, or every background scan blocks on a human. Being a diff test rather than a "who wrote this" test is what makes one rule cover a model's `write_file`, a human-driven `edit_file` and the ingestion loop's own harness write alike. **Amended again 2026-08-07 (audit).** The current content is read in three states, not two: absent, readable, and *present but undecodable*. Folding the third into "absent" disarmed every rule below it for exactly the files most likely to hold something a human typed — a note saved in an editor whose default encoding is not UTF-8 stopped gating its own overwrite — so an unreadable file now yields a sentinel no proposed content can extend, and the write stops for a human — and the loop is additive by construction, so it consults the same predicate before every write and *withholds* rather than landing anything the gate refuses. | README §1.1 L44-46, §1.3 L97-98, §2.3 L537-547; large-source ingestion, gate amendment | error | Filing a note and a *new* reference completes with zero interrupts; appending a bullet to an existing `references/<src>/<src>.md` does not gate; replacing or deleting a line in one yields `REFERENCE_REWRITE` exactly once and carries a diff (RT-34); a target that exists and cannot be decoded also yields `REFERENCE_REWRITE` rather than nothing. **no key** |
...
#### RT-F · Large-source ingestion — `ingestion.py`

**Added 2026-08-07.** The [large-source ingestion design](2026-08-07-large-source-ingestion.md) rules
this behaviour as LS-1 … LS-12; these are the Layer 2 rules those rulings become, with the ids a test
can cite. `pkb.sources` (staging, extraction, sections) is a **leaf below this package** and is not
governed here — LS-7 … LS-9 are its contract.

| ID | Rule | Source | Sev | Test assertion (API key?) |
|----|------|--------|-----|---------------------------|
| RT-60 | **The reading is a harness loop, not an agent turn.** `ingestion.ingest` walks `ExtractedSource.sections` in order and asks the expert one bounded question per section; the model contributes judgement about one section, never the decision that the reading is finished. The window is a **section**, not a byte range (LS-9/LS-10) — a section longer than `SECTION_WINDOW_CHARS` is asked about in several windows and the answers are still filed under that section, so the reconciliation anchor stays the source's own structure. Same lesson as routing (LB-15): a step that always runs beats an instruction. | large-source ingestion LS-9, LS-10; README §2.3 | error | A three-section source with a scripted model: every section title appears in the questions the model was asked; a section the model answered `NOTHING` for was still asked about. **no key** |
| RT-61 | **Write as you go, and the file is the resume state.** The file on disk reflects each section as it lands, so a run that dies at section 14 leaves fourteen sections of work behind — and the reading record in that file, not a second store, is what a re-entry reads to continue from the first section it never opened. A second source of truth about progress is a second thing that can be wrong, and the one a human can check is the file. A pass is *complete* only once every section is accounted for; an incomplete last pass is resumed, a complete one starts the next pass. | large-source ingestion, "resumability"; LS-5 | error | A scripted run that raises mid-way leaves the earlier sections filed, no `Pass complete` line, and the later section named nowhere as read; the resumed run asks only about the sections it never opened. **no key** |
| RT-62 | **Gainful means at least one argument (LS-6).** Nothing is written until the first argument exists: a topic that takes nothing from a source leaves **no trace at all** — no reference folder, no stub file, no copy of the original — rather than an empty folder implying the source was considered and is somehow relevant. | LS-6; README §1.8 rule 4 | error | A whole-tree byte comparison across a run whose every answer was `NOTHING`: identical outside `<kb>/.inbox/`, and the touched set is empty. **no key** |
| RT-63 | **The map records the reading.** Per pass: which sections gave the topic something, which were read and gave nothing, which gave only something no pass may apply (a rewording, a contradiction), which yielded no text, and — by the absence of a completion line — which were never reached. Read-and-took-nothing is kept **distinct** from never-opened, because only one of them means someone must go back. A source read three times carries three readings' worth of record. | LS-5; §6.9 | error | The reading record of a two-pass file parses back to two entries whose section sets equal what each run reported. **no key** |
| RT-64 | **Ingestion is explicit, and a repeat is offered rather than assumed (LS-11).** `PkbRuntime.ingest` and the expert's `ingest_source` tool are the two entry points, and once either is entered the loop cannot be skipped or shortened by the model. When the topic already holds a **complete** reading of this origin the runtime reads nothing and returns an offer for the human to decide, answered from `<kb>/.inbox/<slug>/source.json` so the question costs no fetch and no extraction; an **incomplete** reading resumes without asking, because nobody chose to stop there. The Librarian carries no ingestion tool: it holds no write capability (RT-16) and no lens to read through (LB-5). | LS-11, LS-4; RT-16 | error | A second `ingest` of a completed source reports `offered_reingest` with zero model calls and re-reads only with `confirm=True`; an interrupted reading resumes unasked; `ingest_source` is absent from the Librarian's tools and present on every expert. **no key** |
...
| RG-20 | The registry API is **thread-free**: no method takes or returns a `thread_id` or `run_id`, and none knows that routed work exists. Deriving an expert thread from a Librarian thread is `routing.py`'s job, not the catalog's. **Amended 2026-08-07**: the public surface is `list_agents`, `get`, `invalidate`, `chat_model_for` — `subagents()` is retired with RG-7, and `chat_model_for` is added because RG-21 makes the model-with-failover a *registry* property and the ingestion loop is a third consumer of it. It reached for `init_chat_model` itself and so became the only path in the system with no fallback, on the operation that makes the most sequential calls; RG-21's own argument — there are two factories and both would have to remember — applies to every later consumer, so the answer is a method to call rather than a rule to follow. | arch §4 L175-181, §5 L226-236; RG-7, RG-21 | error | `inspect.signature` over every public method: no `thread_id`/`run_id` parameter; the public method set equals those four. A runtime ingestion asks the registry for its model. **no key** · built |
...
| LB-5 | The Librarian carries **no topic-scoped breadth files and no topic skill overloads**: `skills=["/skills/", "/kb/skills/"]` only. It goes wide; experts go deep. | README §2.2, System Goal, §2.4 | error | No path under a topic root appears in the Librarian's breadth block or skill sources. **no key** · built |
...
| ID | Rule | Source | Sev | Test assertion (API key?) |
|----|------|--------|-----|---------------------------|
| SK-1 | **Amended 2026-08-07 (large-source ingestion): the set is now exactly ten.** The implementation ships ten starter skills, one directory each: `summarization`, `conflict-detection`, `tag-proposal`, `ingestion-classification`, `ingest-book`, `ingest-paper`, `sub-topic-proposal`, `voice`, `discovery`, `research`. README §2.4's "others as needed (e.g. interviewing)" is deferred. The two extraction skills are the per-kind source shapes README §1.9 lists (§6.9, §6.10); the article and manual shapes named there are still deferred and would move this count again. The set is pinned by a test, so a count and a directory listing must move together — a skill shipped without the constant fails this rule's test, and a constant without the skill ships a name no expert's prompt ever reaches. | README Part 3 L657-661, §2.4 L549-566, §1.9 L435-448; large-source ingestion, "what each layer has to change" | error | `set(DEFAULT_SKILL_NAMES)` equals the shipped set and matches the packaged directory listing. **no key** |
...
| SK-9 | Shipped bodies address KB files **by role relative to the topic root** ("the topic's `notes/summary.md`") and never hardcode a topic name, a KB root, or the `/kb/` mount prefix. The topic-specific paths come from the expert's prompt, which alone knows its topic. | README §2.4 L549-573; layer1 CX-3 | warning | `grep -L '/kb/' pkb/agents/skills/*/SKILL.md` matches all ten; no body names a concrete topic folder. **no key** |
...
| SK-11 | Every shipped body **ends by naming the human approval gate it terminates at**, and no shipped skill ends in a state where the agent has finalized human-curated content on its own. | README §1.6 L264-267, §1.8 rule 3, §1.3 footnote 1 | error | Each of the ten has an explicit approval step section. **no key** |
...
| PR-3 | The expert prompt distinguishes **inbound information** (ingestion path: classify, draft, file, gates) from **inbound questions** (retrieval path: breadth files for broad questions, depth files for specific ones — and it writes nothing). The ingestion path as described is **one turn**, which is right for a link and wrong for a book: a source too large to read in one turn hands off to the chunked, resumable workflow instead (large-source ingestion, Layer 2's changes), and the prompt must say so rather than letting the model decide it has finished reading. **Amended 2026-08-09 (Layer 5 §9.5, defect 2): the ingestion path names both of its provenances** — material routed here by the Librarian, and material the human handed this expert directly in a conversation with it. One prompt states both; the wording is never switched on the thread's shape, because `registry.get(agent_id)` caches one compiled graph per agent (RG-6) and a per-thread prompt is a second graph for the same expert — and because a conditional on provenance is the `if origin_channel == …` mistake RO-22 already refuses. | README §2.2 L495-497, §2.3 L527-531, §1.8 rule 2; large-source ingestion; L5 §9.5 | error | A scripted question-shaped turn produces zero writes and zero `ScanRequest`s; the rendered expert prompt names both provenances and contains no claim that is false on a direct thread. **no key** |
| PR-9 | **Added 2026-08-07 (decision G).** The expert prompt states two things the fan-out makes necessary and nothing else covers: that material routed to it may also have gone to other topics and it should extract **what its own topic cares about**, not everything in the source; and that **declining is a correct answer** when the material holds nothing for this topic — filing something to look useful is the failure mode, not filing nothing. Without the first, two experts handed one book produce two near-identical files and the human concludes fan-out is duplication. Without the second, a fan-out's least-relevant branch always files. **Amended 2026-08-09 (Layer 5 §9.5, defect 2): the first clause is scoped to the routed case, and the direct case is stated beside it** — when the human handed this expert the material itself, no other expert holds the rest of it, so the expert takes everything in it that belongs to this topic. Measured harm of the unscoped wording: three of the four human surfaces open a *direct* thread on an expert, and a model told that "the same item often reaches several topics at once" deliberately under-extracts on all three — while this rule's own decline clause then makes filing nothing a *correct* outcome for material the human handed it on purpose. | decision G; README §2.2, §2.3; L5 §9.5 | error | Both clauses present in the rendered expert prompt, the fan-out clause conditioned on the Librarian having routed it, and the direct case named; a scripted decline completes with zero writes and a stated reason. **no key** · designed |
...
| PR-8 | The conflict-scan prompt used by `run_scan` names the `conflict-detection` skill, the topic under scan, and the three comparison axes, and states that the run must not answer a user question. | README §2.8 (*What the scan compares*; the axes moved out of §1.7 when the 2026-08-10 merge put the machinery in §2.8, and §1.7 keeps the conflict types the finding is classified by); arch §7 L355-357 | warning | The rendered scan prompt references the skill by name and the three axes. **no key** |
...
class PkbRuntime:
    ...
    # large-source ingestion — a workflow, not a graph stream (RT-60 … RT-64)
    #
    # Reads one source into one topic section by section. The same loop the expert's
    # `ingest_source` tool enters, so a transport and an agent cannot get different behaviour.
    # `confirm` answers LS-11's offer; `maintain=False` is the tool path, which hands the written
    # paths to `kb_touched` and lets the graph's own single flush stamp them (MW-20).
    # `IngestionReport` is frozen primitives and crosses no harness type.
    async def ingest(self, agent_id: str, origin: str, *, confirm: bool = False,
                     refresh: bool = False, maintain: bool = True) -> IngestionReport: ...
```
...
Ten directories of package data at `pkb/agents/skills/<name>/SKILL.md`, mounted read-only at
...
**Added 2026-08-07 (large-source ingestion, SK-1).** Numbered after the collaboration skills rather
than beside §6.4, which hands off to it, because §6.2 … §6.8 are cited by id elsewhere in this
document and in the Layer 3 rules; renumbering them to make the families contiguous would silently
...
**Outline.** The loop hands over the sections; the model does not decide when the reading is
finished, and never writes about a chapter it was not shown. The file's shape (LS-10): thesis, one
section per chapter *that gave this topic something*, named as the book names it — the chapter is the
anchor a re-ingestion matches on, so no machine ids are needed in a file a human reads — and a final
`Across the book` section. Inside a section, one bullet per **argument**, not per chapter (LS-2).

The judgement this skill exists to carry is **what not to extract**. A chapter holding nothing for
this topic yields nothing: no section, no bullet, and no line explaining that the chapter was about
something else. The common failure is an agent that feels every chapter is owed an entry, and a file
of twenty entries of which six are real is worse than a file of six, because a reader cannot tell
which six. A management book read for a topic about raising children produces parenting arguments and
stays silent on org design (decision G's lens, at chapter granularity). Read-and-took-nothing is kept
distinct from never-opened — both leave no section, and only one of them means someone must go back.

Then: what an argument looks like written down (claim, the reasoning the book offers, the conditions
it depends on) rather than a table-of-contents line; the §1.3 boundary — an extracted argument is a
**reference**, and "we tried this and it worked" is the human's note to write; provenance linking the
copied original beside the file, which MA-8 flags as an orphan otherwise; the reading record, which is
where coverage and skips are stated per pass; and the four reconciliation outcomes of LS-5, compared
section by section. Zero insights leaves **no trace at all** (LS-6). Terminates at the re-ingestion
gate (RT-31 as amended) — one proposal for the whole reconciled file, never one per chapter — with
the incidental new-tag (RT-25) and extension-folder (RT-28) gates named as firing regardless.

### 6.10 `ingest-paper`

**Added 2026-08-07 (large-source ingestion, SK-1).**
...
**Outline.** The five-part skeleton in order — question · method · results · limitations · *does this
apply to me* — anchored on the paper's own sections exactly as chapters anchor a book. The question is
narrower than the title; the method carries enough that a reader can tell whether it transfers, because
nobody re-reads the paper to recover it; results are the paper's own numbers and direction, never "it
worked"; limitations separate the authors' stated caveats from what the extraction noticed they omit.

**Why the last part earns its place**: a result is only knowledge here once someone has said whether it
applies to this human's circumstances, and the judgement names the mismatch that drives it. "Probably
not, and here is why" is a good answer, and it is marked as an argument the extraction is making rather
than a finding the paper reports — the part most likely to be wrong, and the human's to correct.

The boundary is stated plainly: the paper's **claims** are what the paper found, not the human's
experience. A finding is not promoted into a rule — rules distilled from experience live in the topic's
notes and their summary and belong to the human — and one study is one study. What not to extract:
related work is somebody else's paper, and a paper whose question this topic does not care about yields
nothing. Terminates at the same re-ingestion gate as §6.9, and states that nothing in it authorizes a
write into the topic's notes or summaries.
...
| **shipped skills** (`test_shipped_skills.py`) | package data | the ten names; `name == dir.name` and the charset (SK-2); description ≤ 1024 with a trigger clause (SK-7); `validate_content` clean (SK-6); no `/kb/` in any body (SK-9); each names its approval gate (SK-11); the draft footer (SK-12); adoption is refuse-to-overwrite with a fork notice (SK-4, SK-5); after adopting all ten, `regenerate_all` output is byte-identical (SK-17). |
...
| **large-source ingestion** (`test_ingestion.py`) | `tmp_path` KB + generated text sources + `ScriptedChatModel` | every section is asked about and only what landed is filed (RT-60); the file validates clean under the *unmodified* Layer 1 validator and lands `status.draft` (LS-12); a topic that takes nothing leaves the tree byte-identical (RT-62); the original is copied, linked and touched, and `find_orphans` stays empty (LS-1, MA-8); a run that dies leaves its work and says what it missed, and the resume asks only about what it never opened (RT-61); the four reconciliation outcomes — lands, kept, flagged-not-applied, `status.conflict-review` — each asserted on the real file (LS-5, LS-12); a hand edit between passes survives and still does not gate; an addition does not gate and a replacement or deletion yields `REFERENCE_REWRITE` (RT-31 as amended); the tool runs the whole loop and the topic index lists what it wrote (MW-18/MW-20); a completed reading is offered rather than re-read and an interrupted one resumes unasked (RT-64). **no key** |
