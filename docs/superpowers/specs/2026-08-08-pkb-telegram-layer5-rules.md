...
**Date**: 2026-08-08, extended 2026-08-09 (§9, §10)
**Status**: **§1–§9 are built** (see §8 "As built" and §9.13); **§10 "A menu of channels" is
designed, not built.** Every package and built-code assumption in §1–§7 was **executed** in this
repo's venv on 2026-08-08, before a line was written; see §2. §9's Bot API assumptions were probed
against the real bot on 2026-08-09, after the human enabled Threaded Mode, and one of them did not
hold; see §9.1 (F-8 … F-12).
...
| **designed** | Specified here, not implemented. Every `TG-5 … TG-71` rule carried this until §8; every `TG-72 … TG-95` rule carried it until §9.13; the `TG-96 …` rules of **§10** carry it now. |
| **measured live** | Executed on 2026-08-09 against the deployment's own bot on `api.telegram.org`, outside the suite. F-8 … F-12 carry it, and F-2's amendment records the one prediction the measurements overturned. |

> **§9 "Topics: a channel per expert" (2026-08-09) is built** (§9.13). §1–§8 describe the adapter
> before it, in which a *chat* mapped to an agent, and one bot gives one human one private chat, so
> one human reached one agent. §9 makes the addressing unit the **channel** `(chat_id, topic_id)`
> using the private-chat topics of Bot API 9.3/9.4, amends the §1 rules it touches in place (each
> marked **Amended 2026-08-09**), and records the four approved defect amendments. **`pkb.tui` and
> `pkb.clients` are untouched by it.** Read §9.1 before §9.3: a deleted topic reaches the daemon
> through no update at all (F-5), and a send is the only thing that finds out.

> **§10 "A menu of channels" (2026-08-09) is designed, not built.** `/channels` with no arguments
> becomes an inline keyboard, one row per agent, and a tap opens that expert's channel. It touches
> `pkb.server.telegram` alone: no table, no `BotApi` method, no `/health` field.
...
still reads as the design it was and the build has a rule to cite. Every Bot API fact below comes
from the official changelog, from an issue report, or from a probe **executed against the real bot
on 2026-08-09**; see §9.1. **The human enabled Threaded Mode on 2026-08-09 and the first live probes
ran the same day** (F-8 … F-12). They confirm creation, addressing and the daemon's startup path,
and they contradict F-2: §9.3.3 is reframed in place so TG-83 carries the weight and TG-80 stands as
defence.
...
**F-1 … F-3 come from the official changelog and from two issue reports. F-4 … F-12 were executed
against the real bot**, F-4 before the human enabled Threaded Mode and F-8 … F-12 after, on
2026-08-09. `live` marks a fact only the real API can settle; those become `@pytest.mark.live` tests,
deselected by default exactly as §6.5 already does.
...
| **AC** | **A send is how the daemon learns a topic died, and every send that carries a topic checks.** A `400 message thread not found` (F-3, and the live answer as of 2026-08-09 per F-11) and a `message_thread_id` on the response that differs from the one sent (F-2) are one fact with two shapes. Either way the channel is repaired at most twice and then retired; the second shape also leaves a delivered message, whose keyboard is cleared **first**. **Amended 2026-08-09**: §9 wrote this decision around the response comparison, because #854 was the only evidence available. The measured API errors instead, so the error branch is the one that runs and the comparison is the guard behind it. The decision's substance is unchanged, and both shapes stay handled. | F-2, F-3, F-11 (executed). This is the whole design and it is worth restating as the failure it prevents: the human deletes the Cooking topic; the bot keeps sending Cooking's replies and Cooking's **approval keyboards** with a dead `message_thread_id`; Telegram silently drops them into General, where they are indistinguishable from the Librarian's; and the human taps Approve on an irreversible write attributed to the wrong expert, with no undo (D6). Nothing in the error path can detect this — the send returns `ok: true`. Clearing the stray keyboard before the repair, rather than after, is the ordering that matters: a message is dangerous only while its buttons are live, `clear_keyboard` needs the `message_id` the very same response just handed back, and the repair involves a `createForumTopic` and a re-send that can each fail. |
...
*Every rule here is about a topic the human deleted and a bot that has no way to hear about it
(F-5). The daemon discovers the deletion on its next send, and §9 was written believing that send
would succeed and land in General.*

**Reframed 2026-08-09, after the first live probes.** §9 presented TG-80 as the load-bearing
mechanism, on the strength of tdlib/telegram-bot-api#854 (F-2). Four measurements against the real
bot say otherwise: a send into a deleted private-chat topic answers `400 Bad Request: message thread
not found`, with a topic deleted by the bot and with one deleted by the human, with an inline
...
| TG-86 | **The command surface becomes six: `/new`, `/threads`, `/agents`, `/pending`, `/cancel`, `/channels`.** The five existing commands act on the **channel** they were typed in — `/new` rotates that channel's thread, `/threads` lists that channel's agent's threads, `/agents` names that channel's agent and lists this chat's channels, `/cancel` cancels that channel's most recent run. **There is no `/talk`, no `/connect`, and no in-band agent selector of any kind.** **Amended 2026-08-09 (§10): the surface stays at six and the picker adds no command.** `/channels` with no arguments gains an inline keyboard (TG-96) and becomes the creation surface; **`/agents` gains no keyboard and stays a read**, so it keeps naming this channel's agent and listing where the others live. Two commands drawing one keyboard would mean a press whose outcome depends on which command drew it, and TG-76's *"exactly one code path calls `createForumTopic`"* is what a second creation surface breaks. | TG-39 (amended); decisions Y, AF, AG | warning | `/cancel` in one topic does not touch a run in another; `/new` in General leaves the Cooking binding intact; `/talk` is not a command and is not mentioned in the unknown-command reply; `/agents` sends `reply_markup=None` in every channel and `COMMANDS` still has six entries. **no key** — *Why*: a `/cancel` that reaches the wrong turn is worse than no `/cancel` — it stops a turn the human wanted and leaves the one they were trying to stop writing into a tree with no undo. `/talk` would restore the hidden "current agent" that `/connect` was deleted for; the topic title is visible above the keyboard on every send and a mode is not. |
| TG-87 | **`/channels` is the whole creation surface**, and it is exactly three forms: **`/channels`** lists this chat's channels and the agents that have none; **`/channels <agent-id>`** ensures that agent has a channel here — binding the **current** topic if it is unbound, otherwise creating a new one; **`/channels all`** creates one for every agent with none. Every form **states which of the two things happened** in its reply. With Threaded Mode off (TG-75) all three answer with the BotFather instruction and create nothing. **Amended 2026-08-09 (§10): the no-argument form becomes a picker.** It answers with an inline keyboard, one row per catalog agent in catalog order, each row marked by whether that agent has a channel in this chat (TG-96). A press runs the `/channels <agent-id>` form for that agent, in the channel the keyboard sits in, through the same code (TG-98). The typed forms are unchanged and remain the way a human addresses an agent whose row did not fit the keyboard (TG-99). Threaded Mode off still answers with the BotFather instruction and **no keyboard**: a button that creates a topic in a chat that cannot hold one is a button that fails in the human's hand. | decision AA; TG-3, TG-76, TG-77; F-4; §10 | error | `/channels` in an unbound topic binds *that* topic and says so; the same command in General creates a new one and says so; `/channels all` over a catalog of five with two bound issues three creates; every reply distinguishes bind from create; with topics off, zero creates, one instruction and `reply_markup=None`; the no-argument form carries a keyboard whose row count equals the drawn agent count. **no key** — *Why*: the bind-here form is the only recovery path for a lost SQLite file, because **no API enumerates a chat's topics** (F-5) — without it, a restored deployment can only abandon its existing topics and make new ones beside them. One command with two behaviours is a real ambiguity, and it is paid for by the reply naming which one occurred, after the fact, in the chat. The picker is what makes the typed form reachable at all: `_binding_offer` tells a human to type `/channels <agent-id>` and the phone has no other way to learn an agent id, which the rule's own docstring calls an instruction that cannot be followed. |
...
|---------|-------------------|
| `fake_bot` | **A topic model.** Created topics get ids from a monotonic counter starting at 1; `delete_topic(id)` marks one dead. A send to a dead id 400s with `message thread not found` (F-3), and a `silent_relocation` switch makes it return `ok: true` with the `message_thread_id` **stripped** from the echoed `Message` instead (F-2), so TG-83 and TG-80 are both driven from one fixture. `has_topics_enabled` is settable and defaults to **`false`**, so every pre-topics test keeps running against the pre-topics behaviour by default (TG-75). **Amended 2026-08-09: the two behaviours swap places.** §9 made the silent relocation the default and the 400 the switch, on #854's authority; F-11 measured the 400 four times and the relocation zero times, so the default is now the measured answer and the report keeps its switch. The old default is the shipped `missing_thread_errors` flag inverted, which is a fixture rename plus the tests that set it. **A default that models a behaviour the API does not have leaves the exercised path — TG-83 — driven by a handful of opted-in tests.** |
...
Three facts get `@pytest.mark.live` under §6.5's existing shape, deselected by default and skipped
without `PKB_TELEGRAM_TEST_TOKEN`. **All three were settled by hand on 2026-08-09** (F-9 … F-11);
what remains is to pin them as tests so a Bot API release moves a red bar rather than a deployment:

| Fact | Why a fake cannot settle it | Measured |
|------|------------------------------|----------|
| **A send into a deleted private-chat topic raises `400 message thread not found`** (F-3, F-11) | The foundation of TG-83, which is the detection that runs. Our fake implements it; only the real API proves the fake is right, and if the fake is wrong the section's whole detection mechanism is wrong with it. | Four probes, `ok: false` every time (F-11) |
| **A send into a deleted private-chat topic does *not* return `ok: true` into General** (F-2) | The negative half, and the reason TG-80 is defence rather than the working path. A fake can be made to do either, so only the API says which one to expect. | Zero of four probes relocated (F-11) |
| **`createForumTopic` succeeds in a private chat with Threaded Mode on** (F-1, F-8, F-9) | Before the toggle the real bot answered `400 the chat is not a forum` (F-4). | One call, `message_thread_id: 163395` (F-9) |
...
* **No `@pytest.mark.live` test exists**, here or in §6.5. The default suite collects zero and opens
  no socket, which is what both sections promised; the live half is still missing. §9 recorded the
  reason as F-4, Threaded Mode being off. **That block is gone as of 2026-08-09**: the human enabled
  the toggle, and the three facts §9.7 wanted pinned were settled by hand the same day (F-8 … F-11).
  Two consequences, both live. The tests are now writable and are the next thing owed to this
  section. And the fake's fidelity on F-2 turned out to be **wrong for today's API** (F-11), so the
  default flips (§9.7) and the assumption the section rested on is replaced by a measurement.
...
* The `@pytest.mark.live` tests of §9.7. **No longer blocked**: the human enabled Threaded Mode on
  2026-08-09 and the three facts were probed by hand the same day (F-8 … F-11). Writing them is
  owed, and one of them now asserts the opposite of what §9 predicted.
* **The fake's default still models F-2's silent relocation.** §9.7 as amended calls for the
  measured 400 to be the default and the relocation to be the switch. Until that lands, the default
  suite drives TG-80 and reaches TG-83 only through the tests that set the flag.
...
## 10. A menu of channels

**Date**: 2026-08-09
**Status**: **Built**, 2026-08-09. Rules TG-96 … TG-101, decisions AG and AH, amendments to TG-86,
TG-87 and TG-77 marked in place. §10.7 records what the build added.
...
§9 left them. The picker draws a keyboard from state the adapter already reads and resolves a press
through the function the typed command already calls.

### Why this section exists

`_binding_offer` states the gap in its own docstring: without the agent-id listing, the human is
told to type `/channels <agent-id>` *"with no way to learn an agent id from a phone, which is an
instruction that cannot be followed"*. §9 closed half of it by printing the ids. The other half is
still typing `topic/cooking/grilling` into a phone keyboard, correctly, to open a channel. TG-87
already narrowed the act to one command; §10 narrows it to one tap.

The human asked for the menu and chose what a row does: **a row creates that expert's channel.**

### 10.1 Decisions applied

Continuing the letter series of §9.2 (last: **AF**).

| # | Decision | Why |
|---|----------|-----|
| **AG** | **A row carries its agent id inline in `callback_data`, not an opaque handle.** The budget is measured at construction and an id too long for it falls back to a digest of that same id, resolved against the live catalog. **A positional index is forbidden.** | TG-57 fixes `callback_data` at an opaque handle for approvals, and §10 has to argue rather than inherit that. Three differences decide it. **Identity against state**: an approval press must reach a live interrupt on a specific thread, measured at 97 bytes (P-28), so no budget carries it and the handle is the only option; a picker press must reach an agent id, which is a catalog name and fits. **Reversibility**: TG-57's row records `resolved`, and that is what makes a replayed approval press stale rather than dangerous over a write with no undo (D6); a picker press creates a topic the human deletes in two taps, a second press creates nothing (TG-77), and the bot removes nothing (TG-78), so there is no answer to go stale. **Where the truth lives**: TG-58 makes the row the authority because an approval cannot be re-derived, while the picker's authorities are the catalog and the directory, both re-read on every press (TG-98). A handle would index a copy of state the press re-reads anyway, cost a durable row per drawn keyboard, and add a failure the design does not have today: a missing row turns a button into TG-58's *"could not be located"* hand-off, for a press whose worst outcome without any row is a re-typed command. A dead picker button is worse than a picker button that works. **A positional index is the tempting encoding and it is the dangerous one**: the catalog changes, so index 7 in a month-old keyboard is a different expert, and the human gets a channel for an agent they did not choose. That is TG-1's mis-file arriving through the affordance built to prevent it. |
| **AH** | **The roster is the keyboard, and a catalog too large for one keyboard becomes more keyboards, bounded and counted.** Past the bound the last message says how many agents it did not draw and names the typed forms. | Three shapes were available and two of them lie. **Silent truncation** leaves a human whose expert is missing unable to tell that from an expert that does not exist, which is the same class of failure TG-3 exists to prevent one layer up. **A paging cursor** carries an offset or a position, and the catalog moves under a keyboard that lives forever, so page 2 after a rename skips an agent or repeats one, silently. More messages carry no cursor at all: every row holds its own identity, so a page drawn a month ago is still correct row by row. The cost is the per-chat send budget (F-7, TG-94), which is why the bound exists and why the numbers are stated rather than left to the catalog's size. Drawing the roster as buttons rather than as text beside them also removes a second ordering that could disagree with the first. |

### 10.2 Rule table — TG-96 onward

Same conventions as §0 and §9.3: stable ids, `no key` / `live` on every assertion, and every rule
states the failure it prevents.

| ID | Rule | Source | Sev | Test assertion (live?) |
|----|------|--------|-----|------------------------|
| TG-96 | **`/channels` with no arguments answers with an inline keyboard, one row per catalog agent, in the order `service.list_agents()` returned them.** The adapter applies no client-side sort and no re-ranking: not alphabetical, not unbound-first, not most-recently-used. Each row is one button whose label is a marker plus the agent id, `+` for an agent with no channel in this chat and `✓` for one that has a channel here. The message body is one instruction line; the roster is the keyboard, so the message carries no second listing. A label longer than `PICKER_LABEL_UNITS` is cut by TG-44's UTF-16 arithmetic **keeping its tail**, with the cut marked. An empty catalog gets one line and **no** keyboard, never an empty keyboard. | TG-87 (amended); TG-40's *"server order, verbatim… the bot applies no client-side sort"*; TG-55; TG-44; decision AH | error | The button order equals the `list_agents()` order for a catalog deliberately returned out of alphabetical order; a bound agent's row carries `✓` and an unbound one `+`; the body contains no agent id; an 80-character slug's label is cut to `PICKER_LABEL_UNITS` units with the leaf still visible; an empty catalog produces `reply_markup=None` and one line. **no key** — *Why*: the catalog order is the order the TUI sidebar and `/agents` show, and a phone that re-ranks makes the human's two views of one knowledge base disagree about where an expert sits. TG-40 already ruled this for `/threads` and gave the reason: re-sorting buries the row the human came back for. The agent id rather than the catalog title, because ids are unique by construction (GE-25, RG-11) and titles are not, so two sibling topics both titled *Cooking* would draw two identical rows over two different experts. The tail rather than the head, because `topic/` and the upper path repeat down the whole keyboard and the leaf is the part that distinguishes one row from the next. |
| TG-97 | **A row's `callback_data` is `c1\|<agent-id>`, and `c1\|#<digest>` when the id does not fit.** The prefix is the picker's own namespace: a picker press and an approval press cannot parse as each other, and `parse_callback` (TG-57's `v1\|handle\|index\|verb`) rejects the picker's grammar as the picker's parser rejects `v1`. The two grammars are disjoint because no agent id can hold a `\|` (`_SLUG_ALPHABET` is `[a-z0-9-]` and `/` joins the segments), so a picker payload has exactly two fields and an approval payload has four. The budget is checked in bytes at construction against `CALLBACK_DATA_LIMIT`; an id over it is replaced by `#` plus a `blake2b(agent_id, digest_size=8)` hex digest, which is stdlib and adds **no dependency**. **No index, no position, no chat id, no topic id, and no durable row.** | decision AG; TG-57; P-28 (measured); `pkb.core.paths` (`_SLUG_ALPHABET`, `MAX_SLUG_LENGTH = 80`); decision R's undeclared-dependency precedent | error | Property test over agent ids up to five path segments at `MAX_SLUG_LENGTH` each: every emitted `callback_data` is ≤ 64 **bytes**, measured on `.encode()`, and the 65-byte case is pinned as the one the real API answers with `BUTTON_DATA_INVALID`. A fixture asserts `topic/` plus one 80-character slug takes the digest form, so the fallback has a test rather than a comment. `parse_callback` returns `None` for every picker payload and the picker parser returns `None` for every approval payload, both directions. grep finds no integer index and no durable write on the picker path. **no key** — *Why*: agent ids are ASCII by construction, so bytes equal characters here and the arithmetic is exact. `c1\|` costs 3 bytes and leaves **61** for the id. `librarian` uses 9 of them and `topic/cooking/grilling` uses 22. `topic/` plus one legal 80-character slug is **86**, which is 25 bytes over, and the whole payload would be 89 against a 64-byte limit. The over-budget case is reachable with one topic name a human may plausibly type, so shipping the inline form alone would 400 on a deep sub-topic at the moment the human taps it, which is the failure this rule is measured to prevent. Neither PTB nor aiogram enforces the limit at construction (P-28), so the adapter enforces it. |
| TG-98 | **A press re-reads the world and never trusts the drawing.** Order: the allow-list (TG-20, TG-95), then `answerCallbackQuery` (TG-61), then the token resolved against the **live** catalog, then the same `/channels <agent-id>` code the typed command runs, in the channel the pressed message sits in. Five answers create nothing and say so: the agent has left the catalog; the agent already has a channel in this chat (TG-77's pointer); Threaded Mode is off now (TG-75); the chat has left the mapping since the keyboard was drawn; the digest matched two catalog ids. Otherwise the press binds the current topic if it is unbound and creates one if it is not, and **the reply names which happened** (TG-87). **A row pressed twice in quick succession creates one channel**, under TG-77's per-chat lock: a thumb landing twice is one gesture, and the picker is what turns TG-77's race from a fast pair of hands into an ordinary phone. | constraint of a keyboard that lives forever; TG-58's *"any in-memory map is a cache, never the authority"*; TG-77, TG-75, TG-20, TG-61 | error | A keyboard drawn against a five-agent catalog, then pressed after the agent was removed: zero `createForumTopic`, one reply naming the agent, keyboard intact. One row pressed twice concurrently: one create, one directory row, and TG-77's pointer as the second reply. Pressed after that agent's channel was created by another route: zero creates and the TG-77 pointer. Pressed after `has_topics_enabled` flipped to false: zero creates and the BotFather line. Pressed after the chat left `config.chats`: zero creates and a refusal alert. A press and a typed `/channels <agent-id>` in the same channel produce **byte-identical** replies and the same store calls, asserted by driving both. **no key** — *Why*: a Telegram message keeps its buttons live forever, so the state a row was drawn against is a claim about a moment that has passed. Trusting it creates a topic for an expert that no longer exists, or a second topic for one that already has a channel, which TG-77 calls the human's history split in half with nothing on screen saying which half a message went to. Sharing the code with the typed command rather than copying it is what keeps the bind-or-create ambiguity (Q30) answered the same way through both doors. |
| TG-99 | **A catalog too large for one keyboard becomes more keyboards, and the remainder is counted, never dropped.** The adapter draws at most `PICKER_ROWS` rows per message and at most `PICKER_MESSAGES` messages, numbered with the existing `counter()`. Agents past `PICKER_ROWS × PICKER_MESSAGES` are **not** drawn, and the last message states how many were left out and names `/channels <agent-id>` and `/channels all`. **Silent truncation is forbidden** and so is a paging cursor. | decision AH; F-7 and TG-94 (the budget is per chat); TG-3's *"the daemon reports agents that have no channel"*; TG-45's counter | error | A catalog of 30 draws three keyboards over three messages with `(1/3)` … `(3/3)` and every agent present exactly once. A catalog of 50 draws the bound number and the last message states the count not drawn; the sum of drawn rows plus that count equals the catalog size, asserted as arithmetic rather than as a substring. A press on the second message's rows works identically to one on the first. grep finds no offset, page number or cursor in any `callback_data`. **no key** — *Why*: a human whose expert is missing from the list cannot tell that from an expert that does not exist, and TG-3 exists to make exactly that distinction visible. A cursor would put a position in a payload whose keyboard outlives the catalog it was drawn from, so page 2 after a rename skips an agent silently, which is the same failure reached through the fix for it. The message bound is the send budget: F-7 makes it per chat, so every extra message is a second of the budget an approval keyboard may be queued behind (TG-94). |
| TG-100 | **A picker keyboard is never cleared and its text is never edited.** TG-63 does not reach it: no prompt row exists for a picker message, so `_clear_keyboard` cannot address one, and the adapter never re-marks a row after a press. Every outcome is a **new** message in the channel the press came from. | TG-63 and its amendment of 2026-08-08 (*"buttons and prose are separate concerns"*, and overwriting destroys the chat's only record); TG-77's idempotence; decision AG | error | After a press, the picker message's `reply_markup` is unchanged and no `edit_message_text` and no `edit_message_reply_markup` names it; three presses on one keyboard open three channels; a press on a row whose `✓` is stale still answers correctly (TG-98). **no key** — *Why*: TG-63 disarms an approval because the button commits an irreversible write and a press a week later would answer whatever interrupt is pending now. Neither half holds here: TG-77 makes a second press a pointer rather than a second topic, and TG-78 forbids the bot deleting anything, so there is nothing to disarm. Clearing would also force a human who wants three channels to retype `/channels` between each one, which is the typing this section exists to remove. Re-marking a row would be worse than leaving it: a `✓` written after one press claims the whole keyboard is current, and it is current for exactly one row at exactly one moment. |
| TG-101 | **One press creates at most one channel, and the picker draws no "create everything" button.** `/channels all` stays typed-only. The press path passes exactly one agent argument, so the `all` branch is unreachable from a button by construction. | decision AA; TG-76; Q27; §9.13.5's note that no agent id can be the literal `all` | error | A press issues at most one `createForumTopic`; the keyboard contains no row whose payload resolves to `all`; a fabricated payload carrying two agents is refused rather than iterated. **no key** — *Why*: decision AA rejected the daemon creating a topic per catalog agent at boot, for three reasons that all survive being moved to a thumb. Thirty channels bury the four the human opens, no single action undoes them (the bot deletes nothing, TG-78), and a partial failure mid-burst leaves a chat whose state nothing can reconstruct, since **no API enumerates a chat's topics** (F-5). A typed `/channels all` is a human spelling out the same act; a button one tap from a scrolling thumb is not. |
...
PICKER_PREFIX:      Final = "c1"    # the picker's own namespace; `parse_callback` rejects it (TG-97)
PICKER_BUDGET:      Final = CALLBACK_DATA_LIMIT - len(b"c1|")   # 61 bytes for the agent id
PICKER_DIGEST_SIZE: Final = 8       # blake2b -> 16 hex chars, so `c1|#<digest>` is 20 bytes flat
PICKER_ROWS:        Final = 12      # rows per message   (TG-99, Q33)
PICKER_MESSAGES:    Final = 3       # messages per /channels
PICKER_LABEL_UNITS: Final = 48      # UTF-16 units per button label, TG-44's arithmetic (TG-96)
...
def picker_callback(agent_id: str) -> str:
    ...
    characters and the budget is exact. `topic/` plus one 80-character slug is an 86-byte id
    against a 61-byte budget, so the fallback is reachable with one legal topic name rather than
    a hypothetical one.
    """

def parse_picker(data: str) -> str | None:
    """The token, never the agent. Resolution reads the live catalog (TG-98)."""

def resolve_picker(token: str, catalog: Collection[str]) -> str | None:
    """A digest token matching two catalog ids resolves to ``None`` and the press is refused,
    rather than picking one of them (TG-98)."""

# The press enters `_channels(channel, agent_id, [resolved])` — the same function the typed
# command calls (TG-98). `_on_callback` gains one branch ahead of `parse_callback`, because the
# two grammars are disjoint and the picker's is the cheaper test.

_creations: dict[int, asyncio.Lock]   # one `/channels` at a time per CHAT (TG-77 amended)
```

### 10.4 Test strategy delta
...
| File | What it gains |
|------|---------------|
| `tests/server/test_telegram_picker.py` | New. TG-96 … TG-101, driven against the same `fake_bot` §9.7 describes. The catalog fixture returns agents **out of alphabetical order** on purpose, so a client-side sort fails TG-96 rather than passing by coincidence. |
| the `fake_bot` | One addition: it records `reply_markup` on `send_message` and refuses a `callback_data` over 64 bytes the way the real API does, so TG-97 fails at the fake rather than in production. |
| `tests/server/test_telegram_topics.py` | The TG-87 section gains the no-argument form's keyboard, and the Threaded-Mode-off case asserts `reply_markup=None`. |

Two assertions are worth naming because they are the ones a later change breaks silently:

* **The press and the typed command produce byte-identical replies** for the same agent in the same
  channel (TG-98). A second implementation of bind-or-create passes every other test in the file.
* **Drawn rows plus the undrawn count equals the catalog size** (TG-99), asserted as arithmetic. A
  substring test on the overflow line survives the day somebody changes the bound.

### 10.5 Explicitly out of §10

- **Any store, `BotApi`, `/health` or `pkb.daemon` change.** The picker reads what the adapter
  already reads and writes through `open_channel`, which TG-77 already built.
- **A durable row per drawn keyboard** (decision AG), and any `callback_data` carrying a position.
- **A "create every channel" button** (TG-101). `/channels all` stays typed.
- **Buttons on `/agents`, `/threads`, `/pending` or `/new`** (TG-86 amended). One keyboard that
  creates topics, drawn by one command, reached through one code path (TG-76).
- **Clearing or re-marking a picker keyboard after a press** (TG-100).
- **A button that opens or rotates a thread.** The human chose the creation behaviour for a row; a
  second verb on the same row would put two irreversible-looking acts under one thumb.
- **A keyboard on TG-74's binding offer**, and on the `forum_topic_created` offer TG-92 sends. Both
  are the other messages that tell a human to type `/channels <agent-id>`, so the keyboard reads as
  though it belongs there. It is deferred rather than ruled, because the offer fires on **any**
  message in an unbound topic, including one the human sent by mistake, and a creation button under
  a mistaken message opens a channel nobody asked for. `/channels` in that topic is one word, and
  `_bootstrap` already admits it ahead of routing (§9.13.2).
- **`pkb.tui` and `pkb.clients`**, untouched as in §9.8.

### 10.6 Open questions — Q33 onward

| # | Question | Options | Recommended default | Blast radius if changed later |
|---|----------|---------|---------------------|-------------------------------|
| **Q33** | **Are `PICKER_ROWS = 12` and `PICKER_MESSAGES = 3` the right numbers?** Together they draw 36 agents and leave the rest to the typed form. | (a) 12 and 3; (b) fewer rows and more messages; (c) one message, one keyboard, everything else counted; (d) a bound derived from the catalog size. | **(a)** (TG-99). Twelve rows is about a phone screen and three messages is three seconds of a chat's send budget (F-7). **The uncertainty is honest**: nobody has run this against a thirty-agent catalog on a real phone, so the numbers are reasoned rather than measured, exactly as `MAX_RECREATIONS` was (Q28). (c) is the conservative fallback and costs one line. | Two constants. Changing them changes how many agents a human sees per command and nothing else; no durable state carries the old value. |
| **Q34** | **Should a `✓` row do something other than point at the existing channel?** Today a press on an agent that already has a channel answers with TG-77's pointer. | (a) the pointer, as TG-98 rules; (b) a deep link that opens that topic; (c) no button on a bound row at all. | **(a)**. (b) needs a link format for a private-chat topic that nothing here has measured, and an unverified link in the one message that teaches the human how the feature works is worse than a sentence. (c) makes the keyboard's row count depend on state that changes under it, so a month-old keyboard would have gaps whose meaning the human cannot read. | One reply string. If (b) is ever measured to work, it is additive and TG-98's answer for that case changes alone. |

### 10.7 As built — 2026-08-09

`pkb/server/telegram.py` grew six constants, three module functions (`picker_callback`,
`parse_picker`, `resolve_picker`), three private helpers, `TelegramAdapter._picker` and
`TelegramAdapter._on_picker`. `hashlib` is the one new import. No module moved, `BotApi` gained no
method, the store gained no table, and `pkb/tui` and `pkb/clients` were not opened. The five seam
scans still glob `pkb/server/*.py` and still pass.

Four things the build settled that §10.1 to §10.6 left to the implementer.

**The `all` guard is code, not encoding.** TG-101 argues the `all` branch is unreachable "by
construction" because the picker draws no row for it. A fabricated `c1|all` parses, resolves to the
literal `all` and would have reached `/channels all`, so one tap would open a topic per unbound
agent. `_ALL_AGENTS` is now a named constant that `_channels` branches on and `_on_picker` refuses,
so the guard and the branch cannot drift apart.

**TG-98's stated order is not the order the build runs, and the build is right.** The rule lists the
allow-list, then `answerCallbackQuery`, then resolution. Two refusals have to carry `show_alert`,
and Telegram keeps only the first answer to a query, so `_on_picker` reads the chat mapping and
resolves the token before it answers. Both reads are a dict lookup and the synchronous registry read
`_catalog()` already makes on every command. What the rule protects is intact and the tests assert
it directly: no Bot API call happens before the answer, so `createForumTopic` never runs under a
spinning button.

**TG-77 needed a lock.** Two presses of one row created two topics. See TG-77's amendment for the
measurement and `_creations` for the shape.

**The over-budget refusal lives in a subclass.** §10.4 asked `fake_bot` to refuse 65 bytes of
`callback_data`. `tests/server/test_telegram_picker.py` does it in `PickyBotApi`, a subclass of §9's
`FakeBotApi`, so `test_telegram_topics.py` still matches §9.7 word for word. The adapter's own
construction guard raises before the fake ever sees an over-budget row, which is the order a human
wants.

Measured on the build, against the longest agent ids `pkb.core.paths` permits: `topic/` plus one
