...
A rule can be verified about the packages and still only designed as behaviour: `AP-6`'s run
supervisor rests on a verified fact (an anyio cancel scope destroys the runtime's `try/finally`
flush, P-2) and an unbuilt mechanism.
...
| SV-14 | `get_thread(thread_id) -> ThreadDetail` carries the row plus: the thread's `AgentDescriptor`, `messages` from `runtime.history`, `pending` read **live** from `runtime.pending_approval` (never from the column), and `children`. One call must be enough to render a conversation and its approval, because that is all a client re-attaching from a second channel has. | arch §5 L276; RT-38; arch §8 L502 | error | Interrupt a run, restart the service over the same SQLite file, `get_thread` returns the live `ApprovalRequest` with rendered `ActionView`s. **no key** |
...
| SV-25 | **A title is never written by a model.** The service's only model surface is `run`/`resume`. A titling call would be a second unbudgeted model call per thread on a deployment with three concurrent cloud slots (Q6), and it is the same class of move LB-18 forbids — text presented as the system's own that no deterministic step produced. | LB-18 (the ethos); Q6; arch §5 L325 | error | A `ScriptedChatModel` with an exhausted script is never called by `create_thread` or by the first `start_run`. **no key** |
| SV-26 | When `title` is empty at creation, it is **derived on the thread's first run** from the human's first message: first non-empty line, whitespace-collapsed, truncated to 60 chars on a word boundary with an ellipsis. Telegram has nowhere to ask for a title before the first message arrives (D9), so "the client names it" cannot be the only mechanism. | arch §5 L325, D9, §6 L369-383 | warning | An untitled thread gets `"how do I sear a ribeye"` after its first turn; a 400-char message truncates deterministically with no model call. **no key** |
| SV-27 | A derived title is written **once** and never overwritten; `set_title(thread_id, title)` from any channel wins permanently. Watching a title change under them makes the thread list unusable as the place to find the approval they left pending yesterday. | arch §8 L508-510; D6 (no undo) | warning | Turn 2 does not change the title; `set_title` then turn 3 keeps the human's string. **no key** |
...
| AP-6 | **A run is never driven from an SSE response handler.** `pkb.service.runs` drives `runtime.run(...)` in a plain `asyncio.Task` that publishes into an `asyncio.Queue`-backed hub; the SSE generator reads the hub and, in its `finally`, issues at most **one** `task.cancel()`. This converts the transport's level-triggered anyio cancel into the edge-triggered one the runtime is already correct under. Without it the runtime's `try/finally` flush is destroyed on every hangup: measured against the real `PkbRuntime`, `flush reports with stamps: []` and `index.md lists the note: False`. | P-2 (verified, real runtime); arch §7's flush guarantee; MW-26 | **error** | Real ASGI driver, hard disconnect mid-run: the note is on disk **and** the topic `index.md` lists it. **no key** |
...
| RO-7 | **`list_threads` returns only threads a human started.** Derived and scan threads are excluded from the flat list and the per-agent list; a derived thread is surfaced as `ThreadDetail.children` of its Librarian thread, and directly by id. Q14(a): flat listing is the worst option — after a week the Cooking expert's list is mostly fragments of Librarian turns with no visible parent. | Q14; LB-9, LB-14; RT-58; arch §5 L333-338 | error | After a two-expert fan-out, `list_threads()` has one row and `list_threads("topic/cooking")` has none, while `get_thread(parent).children` has two and `get_thread(child)` returns real history. **no key** |
| RO-8 | The one exception to RO-7: a derived thread with a **pending approval** appears in the flat list, badged and labelled with its parent. Hiding it breaks the case the daemon exists for — an expert parked at an approval during a fan-out (LB-16/LB-17's `awaiting-approval`) is precisely what the human comes back from a phone to resolve, and the merged reply that named it is long gone. | arch §8 L502, L508-510; LB-16, LB-17; Q14 | error | A fan-out where one expert gates: `list_threads()` returns the Librarian thread and that one derived thread; the other derived thread is absent. **no key** |
...
| MC-18 | **Propose-only is not read-only.** The gate table is the boundary: plain note ingestion (`notes/<t>.md`) and reference depth files are **ungated** and land unattended, while breadth summaries, new tags, `status.approved`, extension folders, `expert.md`/skill overloads, conflict resolution and deletes gate and become proposals. Every tool result must **distinguish filed from proposed**; conflating them makes an external agent believe a summary update landed when it did not. | RT-23 … RT-31, RT-42; README §1.1 goal 3 | error | A propose-only run writing one note and gating one `notes/summary.md`: the result names one proposal, the note exists on disk, the summary does not. **no key** |
...
| ST-5 | `threads` columns are exactly arch §5's seven — `thread_id` TEXT PRIMARY KEY, `agent_id` NOT NULL, `title` NOT NULL (may be empty), `created_at` NOT NULL (ISO-8601 UTC), `updated_at` NOT NULL, `origin_channel` NOT NULL, `pending_interrupt_id` NULL — **and nothing else**. In particular there is no `parent_thread_id` and no `kind`: both are pure functions of the id (LB-14), and storing what is derivable is the class of duplication RT-36 rejects. | arch §5 L325-327; RT-36; LB-14; decision D | warning | Schema introspection asserts the exact column set; child lookup uses the derivation. **no key** |
| ST-6 | `Thread` (the dataclass, not the row) exposes `parent_thread_id: str \| None` and `kind: "user" \| "routed"` as **computed** fields, so a client distinguishes a conversation held directly with an expert from the work the Librarian routed to it **without string-sniffing in the UI**. | arch §4 L256-260; LB-9, LB-14; Q14; decision D | warning | `Thread.parent_thread_id` is None for a minted thread and equals the parent for a derived one. **no key** |
...
| PK-10 | `pkb_implementation_pack(topic, include_subtopics=False, budget_bytes=None)`: **`notes/summary.md` first, always**, then the selected topic's full `index.md`, then `references/<src>/<src>.md` depth files, then `type.solution` notes. The ordering is the rule, not an implementation detail — human rules have the highest priority and an implementation agent reads top-down. `include_subtopics` defaults **False**: README says "the full `index.md` of the **selected** topic", and a four-sub-topic tree would otherwise return an order of magnitude more than was asked for, which is the context-window problem goal 2 exists to solve. | README Part 4; §1.3 L101; PK-2; goal 2 | error | `pack.entries[0].path.endswith("notes/summary.md")` for every fixture topic, including one whose file is an empty placeholder. **no key** |
| PK-11 | A pack carries a **size budget** and truncates deterministically **at an entry boundary, never mid-file**, reporting exactly which entries were omitted and why. Packs are the one MCP result that can be arbitrarily large — a topic with forty ingested references produces an implementation pack no context window holds — and a silently clipped pack is worse than a short one because the consumer cannot tell. | README §1.1 goal 2, Part 4; layer1 GE-12's precedent | warning | With a budget of N bytes over an oversized fixture: entries are a prefix of the unbudgeted ordering, every entry is whole, and `omitted` names the rest with a reason. **no key** |
...
| **C-5** | **Do routed expert threads appear in the thread list?** Arch §5 says *"a routed thread belongs in the human's thread list as a child of the Librarian's"*; RT-58 says a scan thread belongs out of it; Q14 is formally open. | **Children of the parent, excluded from the flat and per-agent lists — with one exception: a derived thread carrying a pending approval also appears flat, badged and labelled with its parent** (RO-7, RO-8). | Q14(a) plus the case arch §8 is proudest of. Flat listing is the worst option the docs already predict; hiding entirely makes "continue with the Cooking expert" a dead end once the merged reply scrolls away. The exception is what keeps "approve from a phone something the TUI asked about hours earlier" true when the approval was raised **inside** a fan-out (LB-16). |
...
|---|----------|---------|---------------------|-------------------------------|
| **Q1** | **Does a dropped SSE connection cancel the run or detach from it?** | (a) disconnect cancels — iterate Layer 2's generator from the response handler; (b) **detach** — a daemon-owned run task publishing into a per-run hub, with `GET /threads/{id}/events` to reattach; (c) (b) plus a replay buffer so a reconnect resumes from `seq` with no gap. | **(b), with (c)'s buffer scoped to the run in flight** (decision A, AP-6 … AP-9, RO-17). (a) is disqualified twice over: it loses the flush (P-2) *and* it makes D2's headline promise conditional on a stable socket — a phone in a tunnel would kill an ingestion turn mid-write — while quietly turning `cancel(run_id)` into something the network does rather than something the human does. | One hub module (~150 lines) plus the discipline that a slow subscriber is dropped rather than allowed to stall the run (AP-8). Changing it later changes every route that streams. **This is the largest structural decision in step 3.** |
| **Q2** | **Do routed expert threads appear in the human's thread list, and where?** (Q14 is still formally open.) | (a) children of the Librarian thread, excluded from the flat and per-expert lists; (b) flat, alongside the human's own expert conversations; (c) hidden until a reply offers one. | **(a), with one exception: a derived thread carrying a pending approval also appears flat, badged and labelled with its parent** (RO-7, RO-8). (b) is the outcome the docs already predict — after a week the Cooking expert's list is mostly fragments of Librarian turns with no visible parent. (c) makes "continue with the Cooking expert" a dead end the moment the reply scrolls away. The exception is what keeps arch §8's headline case true when the approval was raised inside a fan-out. | One `WHERE` clause and one children query. Cheap to change, but it is the difference between the offer being a link and being a sentence.  **RULED 2026-08-07: grouped per expert.** The list is organised by agent — a derived `<thread>::<agent-id>` appears under the expert that ran it, not under the Librarian thread that spawned it, because navigation is by subject. |
...
| **Q4** | **How does a thread come by its title?** Nothing in the README, the arch spec or the Layer 2 rules says. | (a) client-supplied only; (b) model-written (a titling call); (c) client-supplied optional, with a deterministic derivation from the first human message when absent, written once. | **(c)** (SV-25 … SV-28). (b) is out: the service's only model surface is run/resume, a titling call spends one of three concurrent cloud slots per thread (Q6), and text the system presents as its own that no deterministic step produced is the shape LB-18 was written for. (a) alone fails Telegram, where a message arrives before anyone could name anything (D9). Derived threads get a generated, non-editable title. | One function and one column write. The human notices it on every row of the sidebar, which is why it is here rather than buried.  **RULED 2026-08-07: (b) model-written.** A titling call names the thread; see TT-1…TT-4. |
...
| 2 | `GET /threads?agent_id=<exact>` | — | `200 {"threads": [Thread, …]}` — human-started only, `pending DESC, updated_at DESC` |
...
| 4 | `GET /threads/{thread_id:path}` | — | `200 {"thread": Thread, "messages": [MessageView], "pending_interrupt": ApprovalRequest\|null, "children": [Thread]}` · `404 unknown_thread` |
...
| **A ~40-line raw ASGI driver** | The **only** in-process way to assert (a) frames arrive over time and (b) the generator's `finally` runs on disconnect. Build the scope with `spec_version "2.3"` (uvicorn's value), `create_task(app(scope, receive, send))`, read `http.response.body` off a queue, return `{"type": "http.disconnect"}` from `receive`. | This is how the P-2 flush regression is pinned without spinning uvicorn — and it is the highest-value test in the layer. |
...
| **run supervision** (`tests/server/test_runs.py`) | **raw ASGI driver** + real runtime | **hard disconnect mid-run leaves the note on disk AND listed in `index.md`** (AP-6 — the P-2 regression); disconnect does not cancel (AP-7); two subscribers, one dropped, run unaffected (AP-8); cancel yields the synthesized terminal frame (AP-11). |
...
Only judgment, and only where a fake model cannot say anything. **Nothing in Layer 3's own surface
needs one** — every rule in §1 is asserted with **no key**. The live marker covers, and only covers:
...
- The bot's command surface (`/agents`, `/connect`, `/threads`), inline keyboard rendering, message
...
**Never, at any layer**
- **A model call anywhere in Layer 3.** Not to title a thread (SV-25), not to summarize a merged
  reply (MC-9), not to assemble a pack (PK-8), not to choose an agent when an id does not resolve
  (MC-16, MC-19). `grep` for a model client in `pkb/server` and `pkb/service` must find nothing.
...
- **Iterating `runtime.run(...)` from an ASGI response handler** (AP-6). It silently destroys the
  flush guarantee, and the tests that would catch it are the ones nobody writes.
