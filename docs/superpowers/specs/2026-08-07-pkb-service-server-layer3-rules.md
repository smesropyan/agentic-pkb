...
| **Q4** | **How does a thread come by its title?** Nothing in the README, the arch spec or the Layer 2 rules says. | (a) client-supplied only; (b) model-written (a titling call); (c) client-supplied optional, with a deterministic derivation from the first human message when absent, written once. | **(c)** (SV-25 … SV-28). (b) is out: the service's only model surface is run/resume, a titling call spends one of three concurrent cloud slots per thread (Q6), and text the system presents as its own that no deterministic step produced is the shape LB-18 was written for. (a) alone fails Telegram, where a message arrives before anyone could name anything (D9). Derived threads get a generated, non-editable title. | One function and one column write. The human notices it on every row of the sidebar, which is why it is here rather than buried. |
