...
5. **Grow the Knowledge Base by working, as well as by capture**. The human opens a channel on an expert and works
   in it for as long as the work lasts: research a goal the topic cannot meet, try what comes back, report how it
   went over days or weeks, and settle at the end what was learned. Most of that work leaves nothing behind. The
   work that does leave something behind leaves practical knowledge the human earned by doing it (Section 2.7).
...
├── (optional) research/   # A synthesis of what a session read (Section 2.7) – an extension folder
│   └── [goal-title].md    #   the goal, the questions, the sources kept and rejected, the synthesis
...
The same convention applies inside `research/` and inside topic-specific extension folders (e.g.,
`recipes/[recipe-title].md`, or `recipes/[recipe-title]/[recipe-title].md` with media).

`research/` is a topic extension folder, like `recipes/` on Cooking, and the human approves it once: the first
research file a topic files mints the folder, and minting an extension folder is already a decision that waits for the
human (Section 1.9). A topic that has never researched anything carries no `research/` folder, for the same reason a
topic that derived nothing from a source gets no folder under `references/`: an empty folder claims work that nobody
...
| `research/[goal-title].md`        | **AI + Human**³ | A session's synthesis of what it **read** (Section 2.7): the goal, the questions asked, every source kept, every source rejected and why. Rendered by the harness from the session's own file and approved by the human before it lands. |
...
³ A session's running record is **a file in the session's own workspace**, outside the Knowledge Base tree, and it
survives a restart of the daemon. A channel that runs for months compacts its own early trials out of the
conversation, so the expert reads the file rather than the transcript. The Knowledge Base gets a file only when the
human approves one. Every write under `research/` waits for the human, first file and every later one, and the human
...
meaning-carrying file (`topic.md`, the summaries, research records) is **AI-generated, human-curated** – the AI drafts,
the human adds insight and approves. Reference depth files are AI-generated on first ingestion; the human curates them
at the summary level, and approves any later pass that rewrites one. Mechanical files (`index.md`, root `tags.md`) are
...
wording from the session's own file, argues with the human about what it means (Section 2.7), and files the text they
...
1. **Knowledge files** – notes, references, summaries, `topic.md`, research records, and anything in a
...
| `researched` | A session's synthesis of what it read, weighed against the human's own notes (Section 2.7). |
...
score: the human decides which routes outrank which, and Section 1.7 is where that order is written down. A research
...
research record that arrives without one. No such check lands on `notes/`: every note already in the tree carries no
...
- Sessions add no namespace (Section 2.7). A lesson the human earned files as `type.solution`, a synthesis of what a
  session read files as `type.summary`, and `provenance` carries the difference between the two (Section 1.4).
...
- The human opens a channel and works in it (Section 2.7). The session's running record is a file in its own
  workspace rather than in the tree, so a session that stops halfway keeps what it learned and writes nothing.
...
- `/learn` proposes what was learned, in an ordinary message the human can edit by talking back. The harness then
  renders the file and the human approves the exact text before it lands, under `notes/` or under `research/`.
...
1. **Trigger**: A harness maintenance hook schedules a conflict scan whenever files under `notes/`, `references/`,
   `research/`, or a topic-specific extension folder are created **or modified** – an edited note can newly
   contradict a reference that was fine yesterday. A research record enters the scan like any other knowledge file
...
8. **Nothing off the internet becomes a note**: `notes/` holds what the human proved in practice, and the `type.*`
   ...
   `references/` and its synthesis of what it read under `research/`, stamped `provenance: researched`
...
- **Research planning and synthesis** – Turn the human's goal into the questions a search will ask, brief one search
  sub-agent per question, weigh what returns against the topic's notes and references, and draft the session record
...
- **Work one goal over weeks**: The user opens a channel called `Cooking · Brisket Rub` and works the loop in it.
  They cook three times and report back after each. The expert holds the trials in the session's own file,
...
needs sits beside its `SKILL.md` in the same folder. There are two families:

- **Judgment maintenance skills** (Section 1.9, Tier 2) – summarization, conflict detection and classification, tag
  proposal, ingestion classification, the per-kind source extraction skills, and sub-topic proposals.
- **Collaboration skills** – how agents work *with the human*, the same pattern coding harnesses use for their
  skills:
    ...
    - `research/` – how to explore breadth-first, generate options, and present them for direction selection; how to
      run a round that reaches past the Knowledge Base to the internet; and how to hold a channel open across weeks
      of trials and propose at `/learn` what the human learned from them (Section 2.7).
...
A Topic Expert Agent answers from what its topic already holds. Collaborative research covers everything past that:
the goal the topic cannot meet, the trials that follow it, and the lesson the human and the expert settle at the end.
Capture files what the human already knew. This is where they find things out, and it is where most of their notes
come from.
...
A repeat `/learn` files nothing twice. The session's own file records what already landed and where, so a second
...
5. **Verify.** Harness code locates every quotation in the text the search returned for the page it came from, and
   ...
   synthesis, and not thrown away either. They land under their own heading in the session's file, with the reason
   they failed, and in the research record if one ever lands. A human who never sees a fabricated quotation never
...
HUMAN ── opens ──▶ CHANNEL = SESSION   one line of work, named for its goal,
                        ...
                        ▼
  ┌──────────────── a round of research ─────────────────────────┐
  │ 1. take the goal      into the session's own file            │
  ...
  └───────────────────────────┬──────────────────────────────────┘
   ...
   ┌──────┴─────┬──────────────┬─────────────────────┐
   ▼            ▼              ▼                     ▼
 notes/      research/     references/          nothing at all
 what the    what was      the accepted         the common outcome:
 human       read:         article and its      no note, no record,
 tried, and  type.summary  original copy        no folder, no trace.
 it worked:  provenance:   (ordinary            The closed Telegram
 type.       researched    ingestion)           topic keeps the
...
`research/`, the same route as everything else the session read or wrote. The Knowledge Base never grows a task
...
A session keeps its own file and writes into it as the work happens: the goal, each trial and what it produced, the
sources kept and the ones turned down. The file lives in the session's workspace, outside the Knowledge Base tree,
...
Ask for a recipe, get one, `/close`. No note, no record, no folder, no trace in the tree. Discard is an outcome this
...
citation attached to them. So no URL reaches a research record on a model's word:
...
- **Harness code holds the text of every cited page.** Search hands the page's content back with the result, so the
  ...
  that page while it is still searching, and the text joins the same record. Verification itself fetches nothing. A
  citation the record holds no text for keeps its claim out of the synthesis, and the record names the claim and says
...
- **Harness code locates every quotation in the text that page returned.** A quotation the text does not contain is
  dropped, and the record says a quotation was dropped. The comparison runs against the bytes the claim came from,
  never against a page fetched again later and possibly rewritten in between.
...
A session that read enough to be worth summarising produces one file:

```
research/[goal-title].md
```

It follows the folder-hosted convention of Section 1.2 when it needs media beside it. It is a knowledge file
(Section 1.4) with `source_type: summary` and the tag `type.summary`, because a research record is an overview of
what one enquiry found. It carries `provenance: researched` (Section 1.4), which is the field that keeps it
distinguishable from a note the human earned. The tag namespaces of Section 1.5 do not grow for research: a tag would
only restate the folder, and it would disagree with the folder the first time somebody moved a file.

The record holds, in order: the goal, the questions the session asked, every source it kept, every source it rejected
and why, the conflicts it raised against the topic's notes, and the synthesis.

A session with nothing to summarise writes no record. A session that read one page, cooked from it, and learned one
thing writes a note and no record. The record exists to hold what was **read**, and the note exists to hold what was
**done**.

**The record is append-only.** A turn adds to the end. Nothing rewrites an earlier entry, because a model asked to
...
and a result silently dropped is indistinguishable from a result never found. A session that files no record keeps
its rejections in its own file and in the closed channel, which is where everything else it discarded stays too.

**Candidates live with the session, not in the tree.** A page the search returned and the human has not accepted is
held with the session: the text goes when the research that found it ends, and the record of it goes when `/close`
ends the channel. It is not staged, not copied, and not written anywhere under the PKB root. `.inbox/` is where an
...
**Nothing lands until the human approves the text.** The session's own file holds the running record and survives a
...
`research/` waits for them, the first file and every later one.
...
| The session's synthesis of what it read | `research/[goal-title].md`, tagged `type.summary`, stamped `provenance: researched` | The human approves the rendered text, before it lands |
...
| A candidate the human rejects          | The rejection list inside the research record, and the session's own file until one lands | Nothing, and no other file changes                    |
| Nothing at all                         | Nowhere: no note, no record, no folder               | `/close`, and this is the common outcome                               |
...
One approval remains, and it sits on the bytes. For each file that would land, note or record, the human reads the
...
│   │   │   └── media/
│   │   └── summary.md
│   ├── research/           # Extension folder – appears with the topic's first approved research record
│   │   └── [goal-title].md
│   ├── (optional) expert.md
│   ├── (optional) skills/
│   │   └── [skill-name]/
...
1. **Default skills ship with the implementation, and are mounted rather than copied in.** The PKB implementation
   provides starter versions of the common skills (summarization, conflict detection, tag proposal, ingestion
   classification, sub-topic proposals, `voice`, `discovery`, `research`). They are loaded from the implementation
   itself, so the Knowledge Base's own `skills/` folder starts empty and an untouched skill improves whenever the
   implementation does. They are functional out of the box but are treated as drafts: when the human wants to change
...
- **Research agents (breadth-first)** receive Research Packs built from `topic.md`, the relevant subtrees of the root
  ...
  the research area, plus any approved research records that touch it. Research agents do not read `index.md` files
...
A research record enters a pack only once it has landed, which means only once the human approved it (Section 2.7).
...
human. **Every write a session makes waits too**, the record and the lesson alike, the first and every later one, and
...
- **Collaborative research** is a channel the human works in for as long as the work lasts, opened on one expert or
  on the Librarian when the work crosses topics. The expert searches with the goal and none of the human's beliefs,
  verifies every URL and quotation in code, weighs what survives against the human's notes without touching one, and
  argues with the human while they can still act on it. Search comes from the model provider the deployment already
  pays and returns page text rather than links, so a quotation is checked against the bytes it came from; an outage
  of that provider stops research and leaves the expert answering from what the topic already holds. They try things
  and report back over days or weeks. `/learn` proposes what was learned, at any time and as often as it is worth
  asking; `/close` ends the channel. Most channels file nothing, which is the ordinary outcome. A channel that files
  something leaves practical knowledge the human earned, as a note stamped `provenance: practised`, and a synthesis
  of what the session read under `research/`, stamped `provenance: researched`.
