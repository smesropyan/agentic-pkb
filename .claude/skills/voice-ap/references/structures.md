...
**Exception: a system that really does act.** In documentation about software, the program is the actor and naming a human would be false. "The compiler rejects the file", "the index regenerates on write" and "the PKB holds three kinds of knowledge" are literal descriptions of what runs, not evasions. The test is whether a person could be substituted without changing the meaning. If a person did it, name them. If code does it, name the code.
...
**Instead:** Name the actor. "The team fixed it that week" beats "the complaint becomes a fix." If no specific person fits, use "you" to put the reader in the seat. If the actor is a system, name the system.
